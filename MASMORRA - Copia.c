...
// PROTOTYPES
    ...
    //int showHighscores(int highscores[]);
    ...
    //void updatesHighscores(int score, int *highscores);
			...
			do{
				...
				switch (in){
					...
					case 2: // SHOW HIGHSCORES
						...
						int highscoresLength = sizeof(highscores) / sizeof(highscores[0]);
						qsort(highscores, highscoresLength, sizeof(int), compareInt);

						for (int i = 0, j = gamesWon; i < HIGHSCORES_NUM ; i++, j--)
							if (j > 0)
								printf("%2i - %i |VENCEU|\n", i + 1, highscores[i]);
							else
								printf("%2i - %i\n", i + 1, highscores[i]);
...
// FUNCTIONS
	void shopInteraction(){
		...
		printf("[1] %s - $%i\n[2] %s - $%i\n", shop[currentFloor].item1Name, shop[currentFloor].item1Priece, shop[currentFloor].item2Name, shop[currentFloor].item2Priece);
		...
		switch (currentFloor){
					...
					switch (in){
						...
						case 1: // BUYS ITEM 1
							if ((item1Avaible == true) && (hero.coins >= shop[currentFloor].item1Priece))
							{
								...
								printf("Impossível realizar a ação.\n");
							break;
			...
			case 1 ... 5:
				printf("[3] Poção de cura - $5\n[4] Pingente de vida - $15\n[5] Sair da loja\n(Você tem %i moedas)\n", hero.coins);
				do{
					scanf("%i",&in);
					switch (in){
						case 0: // INFO
						...
						case 3: // BUYS POTION
							if ((hero.coins >= 5) && (hero.hp < hero.maxHp)){
			...
			if (rand() % 3 == 0 && !run){
				...
				switch(currentFloor){
					case 1:
						...
						break;
					case 2:
						chest.maxHp = rand() % 4 + 5;
						break;
					case 3:
						chest.maxHp = rand() % 3 + 6;
						break;
					case 4:
						chest.maxHp = rand() % 3 + 7;
						break;
					case 5:
						chest.maxHp = rand() % 3 +  9;
						break;
				}
    ...
    void shopDialog(){
 	...
 	void dungeonDialog(){
 		switch (currentFloor){
 			...
 			case 6:
				...
				printf("Você desce as escadarias da masmorra, chegando finalmente ao trono do todo poderoso Xãopai.\n");
 	...
 	void setsShopInfo(shop_t *shop){
		...
		strcpy(shop[5].item2Name,"Égide Mítico");
 	...
 	void setsMonsterInfo(enemy_t *monster){
		...
		srand(clock()); monster[2].maxHp = rand() % 1 + 4;
		...
		srand(clock()); monster[3].def = rand() % 2 + 1;
		srand(clock()); monster[3].coins = rand() % 5 + 6;

		strcpy(monster[4].name, "Esqueleto");
		srand(clock()); monster[4].maxHp = rand() %  + 3;
		srand(clock()); monster[4].atk = rand() % 3 + 3;
		srand(clock()); monster[4].def = rand() % 1 + 1;
		srand(clock()); monster[4].coins = rand() % 5 + 7;

		strcpy(monster[5].name, "Guarda da prisão");
		srand(clock()); monster[5].maxHp  = rand() % 3 + 3;
		srand(clock()); monster[5].atk = rand() % 2 + 3;
		srand(clock()); monster[5].def = rand() % 2 + 1;
		...
		srand(clock()); monster[7].atk = rand() % 3 + 3;
		...
		srand(clock()); monster[8].def = rand() % 2 + 1;
		...
		srand(clock()); monster[9].atk = rand() % 2 + 4;
		srand(clock()); monster[9].def = rand() % 2 + 1;
		srand(clock()); monster[9].coins = rand() % 6 + 13;

		strcpy(monster[10].name, "Monge anão");
		srand(clock()); monster[10].maxHp = rand() % 3 + 3;
		srand(clock()); monster[10].atk = rand() % 3 + 4;
		...
		srand(clock()); monster[11].atk = rand() % 2 + 5;
		...
		srand(clock()); monster[12].maxHp = rand() % 1 + 7;
		srand(clock()); monster[12].atk = rand() % 1 + 6;
		srand(clock()); monster[12].def = rand() % 2 + 2;
		srand(clock()); monster[12].coins = rand() % 10 + 20;

		strcpy(monster[13].name, "Succubus");
		srand(clock()); monster[13].maxHp = rand() % 2 + 7;
		srand(clock()); monster[13].atk = rand() % 1 + 6;
		srand(clock()); monster[13].def = rand() % 2 + 2;
		srand(clock()); monster[13].coins = rand() % 10 + 20;

		strcpy(monster[14].name, "Cão demoníaco");
		srand(clock()); monster[14].maxHp = rand() % 2 + 7;
 	...
 	void printsXaopai(){
		...
		printf("             `-._,-'   `-._______.-'   `-._,-'\n\n");	
		//printf("\033[0m");
		printf("Xãopai: ABLUBLUBÉ ABLAUBLUBLÉ! EU COMER %s!\n", hero.name);	
    }
    ...
    void ending(){
		...
		printf("O amuleto pulsa vigorosamente em suas mãos. Você pode sentir um imenso poder saindo dele.\n");
 	...
 	void showsInfo(int monsterCode){
		...
		if (monsterCode > 0 && monsterCode != XAOPAI){
			printf("|%s|\n", monster[monsterCode].name);
			printf("Saúde: %i/%i\n", monster[monsterCode].hp, monster[monsterCode].maxHp);
			printf("Pontos de ataque: %i\n", monster[monsterCode].atk);
			printf("Pontos de defesa: %i\n", monster[monsterCode].def);
		} else if (monsterCode == XAOPAI)
		{
			printf("|XÃOPAI|\n");
			printf("Saúde: %i/%i\n", monster[XAOPAI].hp, monster[XAOPAI].maxHp);
			printf("Pontos de ataque: %i (Dano mágico)\n", monster[XAOPAI].atk);
			printf("Pontos de defesa: 0\n");
		}
 	}

 	/*TODO:
 	- copiar hihgscores da cobrinha 
 	- fazer um jeito de virar mulher*/